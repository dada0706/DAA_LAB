...
void calculateSpaceSaving(struct MinHeapNode* root,
                          int arr[], int top,
                          int* totalOriginalBits,
                          int* totalCompressedBits) {

    if (root->left) {
        arr[top] = 0;
        calculateSpaceSaving(root->left, arr, top + 1,
                             totalOriginalBits, totalCompressedBits);
    }

    if (root->right) {
        arr[top] = 1;
        calculateSpaceSaving(root->right, arr, top + 1,
                             totalOriginalBits, totalCompressedBits);
    }
...
void HuffmanCodes(char data[], int freq[], int size) {
    ...
    calculateSpaceSaving(root, arr, 0,
                         &totalOriginalBits,
                         &totalCompressedBits);
